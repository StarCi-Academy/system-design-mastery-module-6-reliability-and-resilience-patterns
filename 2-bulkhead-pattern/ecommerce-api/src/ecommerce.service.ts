...
export class EcommerceService {
    // Giới hạn chức năng xem lịch sử theo cấu hình env (EN: limit history flow by env-driven configuration)
    private readonly historyLimiter: ConcurrencyLimiter
    private readonly historyProcessingDelayMs: number

    constructor(private readonly configService: ConfigService) {
        const bulkhead = this.configService.getOrThrow<BulkheadConfig>("bulkhead")
        this.historyLimiter = new ConcurrencyLimiter(bulkhead.historyConcurrencyLimit)
        this.historyProcessingDelayMs = bulkhead.historyProcessingDelayMs
    }
